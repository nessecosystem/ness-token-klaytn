...
// Klaytn Contract Library v1.0.0 - https://github.com/klaytn/klaytn-contracts
// Based on OpenZeppelin Contracts v4.5.0 
...
contract NESSToken is KIP7, KIP7Pausable, KIP7Burnable, Ownable {
    constructor() KIP7("CoinNess", "NESS") {
        _mint(msg.sender, 500000000 * 10 ** decimals());
    }

    function burn(uint256 amount) public onlyOwner override(KIP7Burnable) {
        _burn(_msgSender(), amount);
    }

    function burnFrom(address account, uint256 amount) public onlyOwner override(KIP7Burnable) {
        _spendAllowance(account, _msgSender(), amount);
        _burn(account, amount);
    }

    function pause() public onlyOwner override(KIP7Pausable) {
        _pause();
    }

    function unpause() public onlyOwner override(KIP7Pausable) {
        _unpause();
    }

    function supportsInterface(bytes4 interfaceId) public view override(KIP7, KIP7Burnable, KIP7Pausable) returns (bool) {
        return super.supportsInterface(interfaceId);
    }

    function _beforeTokenTransfer(address from, address to, uint256 amount) internal override(KIP7, KIP7Pausable) {
        super._beforeTokenTransfer(from, to, amount);
    }
}